...
internal fun ValidationSession(challengeType: ChallengeType, context: Context, onClose: () -> Unit) {
    ...
    // uiState más abajo, así que cualquier transición de estado recompone la función.
    // Llamar setType() sin guard reemitía Initial en cada recomposición, lo que podía
    // cancelar el LaunchedEffect(uiState) de más abajo en medio de una transición.
    ...
    LaunchedEffect(uiState) {
        when (uiState) {
            ...
            is ValidationSessionUiState.Success -> {
                ...
                CallbackHandler.onCompleted(validationSessionId)
